...
function getDownloadsDir(outputDir) {
  return outputDir || path.join(REPO, 'downloads');
}
...
export default function register(api) {
  async function handleGetBoopEmojis() {
    const categories = [
      {
        name: '表情',
  ...
  async function handleUploadEmoji({ imagePath, animated = false, animationStyle }) {
    ...
    const r = await api.vrchat.uploadImageFile(fileBuffer, imagePath, params);
    if (r.status >= 400) {
  ...
  async function handleUploadPrint({ imagePath, note }) {
    ...
    const r = await api.vrchat.uploadPrint(fileBuffer, imagePath, { note, timestamp });
    if (r.status >= 400) {
      throw new Error(`API error ${r.status}: ${JSON.stringify(r.data).slice(0, 200)}`);
    }
  ...
  async function handleUploadGalleryImage({ imagePath }) {
    ...
    const r = await api.vrchat.uploadGalleryImage(fileBuffer, imagePath);
    if (r.status >= 400) {
      throw new Error(`API error ${r.status}: ${JSON.stringify(r.data).slice(0, 200)}`);
    }
  ...
  async function handleGetPrints({ limit = 100, userId }) {
    ...
    const data = await api.vrchat.fetch(`/prints/user/${targetId}?n=${n}`);

    const prints = Array.isArray(data) ? data : [];
    return {
      userId: targetId,
      total: prints.length,
      prints: prints.map(p => ({
  ...
  async function handleRemovePrint({ printId, confirm }) {
    ...
    await api.vrchat.fetch(`/prints/${printId}`, { method: 'DELETE' });
    return { printId, ok: true };
  }

  async function handleGetGalleryImages({ limit = 100 }) {
    const n = Math.max(1, Math.min(100, Number(limit) || 100));
    const data = await api.vrchat.fetch(`/files?tag=gallery&n=${n}`);
  ...
  async function handleRemoveGalleryImage({ fileId, confirm }) {
    ...
    await api.vrchat.fetch(`/file/${fileId}`, { method: 'DELETE' });
  ...
  async function handleDownloadPrint({ printId, outputDir }) {
    ...
    const user = await api.vrchat.fetch('/auth/user');
    const userId = user?.id;
    if (!userId) throw new Error('Unable to determine current user');

    const data = await api.vrchat.fetch(`/prints/user/${userId}?n=100`);
    const prints = Array.isArray(data) ? data : [];
    ...
    const buffer = await api.vrchat.download(url);

    const ext = extFromContentType(buffer.contentType, url);
    const dir = getDownloadsDir(outputDir);
  ...
  async function handleDownloadGalleryImage({ fileId, outputDir }) {
    ...
    const data = await api.vrchat.fetch(`/files?tag=gallery&n=100`);
    const images = Array.isArray(data) ? data : [];
  ...
  api.registerTool({
    ...
    handler: async () => handleGetBoopEmojis(),
  });
  ...
  api.registerTool({
    ...
    handler: async (args) => handleUploadEmoji(args),
  });
  ...
  api.registerTool({
    ...
    handler: async (args) => handleUploadPrint(args),
  });
  ...
  api.registerTool({
    ...
    handler: async (args) => handleUploadGalleryImage(args),
  });
  ...
  api.registerTool({
    ...
    handler: async (args) => handleGetPrints(args),
  });
  ...
  api.registerTool({
    ...
    destructive: true,
    handler: async (args) => handleRemovePrint(args),
  });
  ...
  api.registerTool({
    ...
    handler: async (args) => handleGetGalleryImages(args),
  });
  ...
  api.registerTool({
    ...
    destructive: true,
    handler: async (args) => handleRemoveGalleryImage(args),
  });
  ...
  api.registerTool({
    ...
    handler: async (args) => handleDownloadPrint(args),
  });
  ...
  api.registerTool({
    ...
    handler: async (args) => handleDownloadGalleryImage(args),
  });
}
