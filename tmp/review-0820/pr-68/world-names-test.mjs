...
export const negativeCache = new Map();
const NEGATIVE_TTL_MS = 50; // 5 分钟
// 定期清理过期条目，防止长期运行下失败 worldId 无限累积
const NEGATIVE_PRUNE_INTERVAL_MS = 50; // 与 TTL 同频即可

/**
 * 移除所有已过期的负缓存条目。
 * 仅在读取与定期清理时惰性调用，避免每次判断都扫描全表。
 */
export function pruneNegativeCache(now = Date.now()) {
